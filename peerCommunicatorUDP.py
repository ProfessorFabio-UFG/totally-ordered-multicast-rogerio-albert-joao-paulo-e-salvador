...
#proposals = {}
delivered = []
nMsgs_global = 0  # Para controlar o fim do roteiro
...
class MsgHandler(threading.Thread):
  ...
  def run(self):
    ...
    handshake_done.set()
    print('Thread Secundaria: Recebidos todos os handshakes. Entrando no loop para receber mensagens.')
    ...
    while len(delivered)  < TOTAL_MESSAGES_IN_SCRIPT:
      try:
        ...
        print(f"Erro no receive, talvez o programa esteja terminando: {e}")
        continue

      sender, msg_content = fields
      hold_back.put((recv_timestamp, sender, msg_content))
      acks[(recv_timestamp, sender)] = set()

      for peer_ip in PEERS:
        send(sendSocket, ('ACK', myself, (recv_timestamp, sender)), (peer_ip, PEER_UDP_PORT))

      while not hold_back.empty():
        top_ts, top_sender, top_content = hold_back.queue[0]

        if len(acks.get((top_ts, top_sender), set())) == len(PEERS):
...
while 1:
  ...
  if mode == -1:
    ...
    if msgHandler.is_alive():
      print("[WARN] Handler não terminou. Pode haver mensagens presas.")
    print(f"========== PEER {myself} FINALIZADO ==========\n")
